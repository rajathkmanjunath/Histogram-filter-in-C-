...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	// your code here
  	...
  	float adjacant_prob = blurring/6.;
  	vector<vector<float> > window {{corner_prob, adjacant_prob, corner_prob},
                          {adjacant_prob, center_prob, adjacant_prob},
                          {corner_prob, adjacant_prob, corner_prob}};
	for(int i=0;i<grid.size();i++){
      for(int j=0;j<grid[0].size();j++){
        	float temp = grid[i][j];
        	for(int dx=-1;dx<2;dx++){
              for(int dy=-1;dy<2;dy++){
                  ...
                  int new_i = ((i+dy)%grid.size() + grid.size())% grid.size();
                  int new_j = ((j+dx)%grid[0].size() + grid[0].size())% grid[0].size();
                  newGrid[new_i][new_j]+= mult*temp;
              }
            }
        }
    }

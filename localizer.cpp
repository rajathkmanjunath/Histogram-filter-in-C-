...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	vector< vector <float> > newGrid (grid.size(), vector<float> (grid[0].size(), 1./(grid.size()*grid[0].size())));
  ...
  float blurring) 
{
  ...
  for(int i=0; i<beliefs.size();i++){
    for(int j=0;j<beliefs[0].size();j++){
      int new_i = ((i+dy)%beliefs.size()+beliefs.size())%beliefs.size();
      int new_j = ((j+dx)%beliefs[0].size()+beliefs[0].size())%beliefs[0].size();
	...
	float p_miss) 
{
  ...
  for(int i=0;i<grid.size();i++){
    ...
    for(int j=0;j<grid[0].size();j++){
      ...
      temp.push_back(beliefs[i][j]*(p_hit*hit+(1.-hit)*p_miss));
    }
